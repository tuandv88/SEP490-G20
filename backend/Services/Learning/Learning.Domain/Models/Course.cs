...
public class Course : Aggregate<CourseId> {
    ...
    public List<UserEnrollment> UserEnrollments = new();
    public string Title { get; set; } = default!;
    public string Description { get; set; } = default!;
    public string Headline { get; set; } = default!; //Tên dưới title
    public CourseStatus CourseStatus { get; set; } = CourseStatus.Draft; // trạng thái khóa học thế nào
    public double TimeEstimation { get; set; } = 1; // ước lượng thời gian để học khóa học này
    ...
    public static Course Create(CourseId courseId, string title, string description, string headline, CourseStatus courseStatus, string prerequisites, 
        string objectives, string targetAudiences, string imageUrl, int orderIndex, CourseLevel courseLevel, double price) {
    ...
    public void Update(string title, string description, string headline, string prerequisites, string objectives, string targetAudiences, double price) {
        Title = title;
        Description = description;
        Headline = headline;
        Prerequisites = prerequisites;
    ...
    public Chapter UpdateChapter(ChapterId chapterId, string title, string description, bool isActive) {
    ...
    public void AddUserEnrollment(UserEnrollment userCourse) {
        UserEnrollments.Add(userCourse);
    }
}
